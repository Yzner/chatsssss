...
def prepare_dataset(data):
    ...
    """
    augmented_data = augment_data(data)
    dataset = [{"text": f"User: {q}\n {a}"} for _, q, a in augmented_data]
    df = pd.DataFrame(dataset)
    train_df, eval_df = train_test_split(df, test_size=0.2, random_state=42)
    logging.info("Dataset prepared with %d training and %d evaluation samples.", len(train_df), len(eval_df))
    return train_df, eval_df
...
def fine_tune_model(train_df, eval_df):
    """
    Fine-tune GPT-2 model incrementally using previously trained models.
    """
    tokenizer = GPT2Tokenizer.from_pretrained("gpt2")
    tokenizer.pad_token = tokenizer.eos_token

    # Load last trained model if available
    model_path = "./fine_tuned_gpt2"
    if os.path.exists(model_path):
        model = GPT2LMHeadModel.from_pretrained(model_path)
        logging.info("Loaded previously fine-tuned model for continued training.")
    else:
        model = GPT2LMHeadModel.from_pretrained("gpt2")
        logging.info("No previous model found, training from base GPT-2.")
