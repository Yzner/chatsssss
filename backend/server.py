...
# import jwt
# import datetime
# from memory_module import ConversationMemoryLSTM
# import torch
...
# gpt_model, gpt_tokenizer = load_gpt_model()
# lstm_memory = ConversationMemoryLSTM(input_size=768, hidden_size=128)
# def process_user_input(user_input, memory):
#     # Tokenize user input to get token IDs, without converting to float
#     input_ids = gpt_tokenizer.encode(user_input, return_tensors="pt")
    
#     # Run through the LSTM memory layer, which now includes embedding
#     memory_output = memory(input_ids)
...
#     cursor = connection.cursor()
#     cursor.execute(
#         "INSERT INTO conversations (session_id, user_id, message, sender) VALUES (%s, %s, %s, %s)",
...
#     # Process input with LSTM memory layer
#     memory_output = process_user_input(user_input, lstm_memory)

#     # Generate response based on user input and memory context
#     faq_answer = find_best_faq_match(user_input, fetch_faq_data())
#     if faq_answer:
#         save_message(session_id, user_id, faq_answer, 'bot')  
#         return jsonify({'response': faq_answer})

#     # Generate GPT-based response using memory context
#     gpt_response = generate_gpt_response(user_input, memory_output, gpt_model, gpt_tokenizer)  
#     save_message(session_id, user_id, gpt_response, 'bot')  
...
def generate_gpt_response(user_input, gpt_model, gpt_tokenizer, lstm_model, lstm_hidden, session_id, user_id):
    # Check if the input matches an FAQ entry using fuzzy matching
    faq_response = find_best_faq_match(user_input, faq_data)
    if faq_response:
        return faq_response  # Return FAQ response if a match is found

    # Check if the user is asking for a repetition
    if detect_repeat_request(user_input):
        # Retrieve the last response directly without generating a new one using LSTM
        past_response = retrieve_past_response(session_id, user_id)
        return "Sure, here is what I said earlier: " + (past_response if past_response else "Sorry, I don't have a recent response to repeat.")
    
    if detect_gibberish(user_input):
        return "I'm sorry, but I couldn't understand your message. Could you please clarify or rephrase your question?"

    # Tokenize and get embeddings for the user input for GPT-2
    inputs = gpt_tokenizer(user_input, return_tensors='pt', padding=True, truncation=True)
    input_ids = inputs['input_ids']
    embeddings = gpt_model.transformer.wte(input_ids)  # Get embeddings
    ...
    # If clarification is needed, return the last response (repeat detection using LSTM)
    if clarify_needed:
        past_response = retrieve_past_response(session_id, user_id)
        return past_response if past_response else "I'm sorry, I don't have a recent response to clarify."

    # Otherwise, generate a new response with GPT-2 if no FAQ match or repetition request is detected
    chat_history_ids = gpt_model.generate(input_ids, max_length=150, num_return_sequences=1)
    new_response = gpt_tokenizer.decode(chat_history_ids[0], skip_special_tokens=True)
